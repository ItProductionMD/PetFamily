...
public class Volunteer : Entity<Guid>
{
    ...
    private const string FULL_NAME = "Volunterr full name";
    public string Email { get; private set; }
    private const string EMAIL = "Volunteer email";
    public Phone PhoneNumber { get; private set; }
    private const string PHONE_NUMBER = "Volunteer phone number";
    public int ExpirienceYears { get; private set; }
    private const string EXPIRIENCE_YERS = "Volunteer expirience years";
    public string? Description { get; private set; }
    private const string DESCRIPTION = "Volunteer description";
    ...
    public static Result Validate(FullName? fullName, string? email, Phone? phoneNumber, int expirienceYears, string? description)
    {
        if (expirienceYears < 0)
            return Result.Failure(Error.CreateErrorInvalidFormat(EXPIRIENCE_YERS));
        return ValidationExtensions.ValidateRequiredObject(fullName, FULL_NAME)
            .OnFailure(() => ValidationExtensions.ValidateRequiredObject(email, EMAIL))
